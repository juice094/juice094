...
**数据科学与大数据技术 · 本科 2027 届** · 上海 / 杭州 / 深圳

📞 13626566112 · 📧 2241470466@qq.com（Gmail 注册中） · 🔗 github.com/juice094

**求职意向**：AI 应用开发 / 后端系统开发 / 数据工程
...
大三在读，独立设计并维护三个生产级 Rust 系统（合计 47 个 workspace crate、27 万行代码、2,500+ 测试全通过）。从 BEP 协议栈到 Agent 内核到编译器管线，均从零构建。具备独立闭环能力：发现问题 → 设计实验 → 实现系统 → 写论文。代码行数：有效代码 ~8 万行，vibe coding ~34 万行。最深刻的教训：在 syncthing-rust 中花了两天排查一个 Windows 文件句柄泄漏 bug——最终发现是 drop 顺序问题，学会用 tracing span 做生命周期审计。
